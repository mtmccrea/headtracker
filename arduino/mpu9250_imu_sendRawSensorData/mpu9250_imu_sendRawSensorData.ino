...
This enables sending raw data in a format expected by 
MotionCal:
https://www.pjrc.com/store/prop_shield.html
Which will create calibration values for the IMU 
(compensating for magnetic field - "soft iron error")
...
{
  /* 
   *  modified from original firmware to only report raw sensor data 
   *  Order taken from 
   *  https://github.com/adafruit/Adafruit_AHRS/blob/master/examples/ahrs_calibration/ahrs_calibration.ino
  */
  
  String imuLog = "Raw:"; // Create a fresh line to log
