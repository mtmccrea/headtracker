...
void setup(void)
{
  ...
  LOG_PORT.println(F("Adafruit Bluefruit Throughput Tester"));
  LOG_PORT.println(F("------------------------------------"));
  ...
  LOG_PORT.println("Switch to DATA mode to have a better throughput ...");

  /* Wait for a connection before starting the test */
  LOG_PORT.println("Waiting for a BLE connection to continue ...");
...
void loop(void)
{
  ...
  if (ble.isConnected())
  {
    ...
    LOG_PORT.println("Connected. Send a key and press enter to start test");
    char command[BUFSIZE+1];
    getUserInput(command, BUFSIZE);

    LOG_PORT.print("Sending ");
    LOG_PORT.print(remaining);
...
//      if (! ble.waitForOK() )
//      {
//        LOG_PORT.println(F("Failed to send?"));
//      }
      ...
      if ( (sent % 2000) == 0 )
      {
        LOG_PORT.print("Sent: "); LOG_PORT.print(sent);
        LOG_PORT.print(" Remaining: "); LOG_PORT.println(remaining);
      }
      ...
      if (!ble.isConnected())
      {
        LOG_PORT.println("Connection lost");
        remaining = 0;
      }
      */
    }
    ...
    LOG_PORT.print("Sent ");
    LOG_PORT.print(sent);
    LOG_PORT.print(" bytes in ");
    LOG_PORT.print(stop/1000.0F, 2);
    LOG_PORT.println(" seconds.");
    
    LOG_PORT.println("Speed ");
    LOG_PORT.print( (sent/1000.0F) / (stop/1000.0F), 2);
    LOG_PORT.println(" KB/s.\r\n");
  }
}
...
void getUserInput(char buffer[], uint8_t maxSize)
{
  ...
  do
  {
    count += LOG_PORT.readBytes(buffer+count, maxSize);
    delay(2);
  } while( (count < maxSize) && !(LOG_PORT.available() == 0) );
}
